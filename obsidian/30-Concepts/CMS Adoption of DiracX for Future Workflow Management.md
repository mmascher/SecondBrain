...
# CMS Adoption of DiracX for Future Workflow Management

## Overview

CMS is in the middle of replacing its [[Workflow Management]] system with one built on [[DIRACX]] (DiracX), the from-scratch re-engineering of the multi-VO [[DIRAC]] framework originated primarily by the LHCb/DIRAC consortium. This is CMS's response to a 2024–2025 review of [[WMAgent]]/WMCore (commissioned by CMS Offline & Computing L1) that concluded the existing system — its core components 15+ years old — is not sustainable for HL-LHC, with knowledge concentrated in too few people and no agreed plan for HL-LHC-era user analysis or user-defined simulation (`WM_Conditions_Workshop_2026.pdf`). The legacy WM system has been frozen since September 2025.

CMS is replacing its legacy workflow-management system, based on [[WMAgent]]/WMCore, with a DiracX-based system for the HL-LHC era. DiracX is intended to provide the higher-level workflow/transformation and task-management functionality currently provided by the CMS workload-management stack.

This does not mean that CMS is replacing its underlying workload execution/submission infrastructure. The existing [[Submission Infrastructure]] — based on [[HTCondor]], [[glideinWMS]], and HTCondor Access Points — remains the CMS batch workload-management layer. DiracX is being integrated with this infrastructure rather than replacing it with DIRAC's own workload execution and pilot-submission mechanisms.
...
InterSeed (also transcribed "InterCEde" in some sources) is a DIRAC-side project, led by Alexandre Boyer, rewriting DIRAC's Compute-Element interface layer as small, typed Python protocols with Docker-based integration tests, intended to serve both DIRAC's own pilot factory and direct job pushes to HPC sites with no external connectivity. Whether HTCondor-G could serve as the backend implementation of the InterCEde abstraction was posed as an open discussion question in CMS presentation material (`Exploring Collaboration Between HTCondor and DiracX-4.pdf`). This would allow to reusing the same mechanism already used by CMS in the glideinWMS factories to submit pilots to CEs. This was separate from CMS user-job submission, which is expected to use condor_submit to the CMS SI Access Points. This idea was discussed in meetings from July through early September 2026 but, per the discovery pass, appears to have been dropped rather than resolved as an active line of work.

InterCEde ( ) was a proposed DIRAC-side abstraction for the CE interface. During July 2026, an approach was advocated to implement its pilot submission using the existing HTCondor-G interface, reusing the same mechanism already used by CMS in the glideinWMS factories to submit pilots to CEs. This was separate from CMS user-job submission, which is expected to use condor_submit to the CMS SI Access Points. The HTCondor-backed InterCEde approach appears not to have been pursued further.
...
- Is explicitly designed to keep [[GlideinWMS Pilot-Based Resource Provisioning]] as the workload execution/resource-acquisition layer underneath DiracX, rather than replacing it.
