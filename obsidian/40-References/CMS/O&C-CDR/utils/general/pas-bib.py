...
class DocListError(DocListException):
    """General  DocList Error"""
    def __init__(self, value):
        self.value = value
    def __str__(self):
        return repr(self.value)
...
class DocList:
    ...
    def getDocInfoFromBib(self):
        ...
        text = f.read()
        mtag = re.compile(r"\s*([\w|-]*)\s*,") # looking for match to document tag
        ftag = re.compile(r",{0,1}\s*(\w*)\s*=\s*") # looking for field inside document listing; optional comma from end of previous entry
    ...
    def getDocInfoFromCDS(self, tag):
        ...
        mtotal = re.match(r'[^\d]+(\d+)\s*$',commentnode.data)
        if (not mtotal):
