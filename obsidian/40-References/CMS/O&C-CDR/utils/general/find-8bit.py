...
def scan8(body):
	...
	pm0 = p0.search(body)
	if pm0:
		p = re.compile("(.*?)([^\\x00-\\x7F]+)(.*?)") # this will work for two-byte UTF8, which covers most characters but not CJK characters or emojis. Those require 3 or 4 bytes. [https://en.wikipedia.org/wiki/UTF-8]
		start = 0
